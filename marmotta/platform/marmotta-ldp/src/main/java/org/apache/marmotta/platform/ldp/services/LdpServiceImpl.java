...
//			statistics.add( factory.createStatement(subject, predicate2, object2));
	...
	@Override
	public void writeResource(final Resource baseUri, Iterable<org.semanticweb.yars.nx.Node[]> nodes, OutputStream output, RDFFormat format) throws RDFHandlerException {

		final RDFWriter writer = Rio.createWriter(format, output);
		
		
		String namespace = new URIImpl(baseUri.stringValue()).getNamespace();
		...
		for (org.semanticweb.yars.nx.Node[] node : nodes) {
			...
			try {
				...
				subject = factory.createURI( namespace + node[0].getLabel());
			}
			...
			try {
				...
				// relative URIs are difficult for the writer
				object = factory.createURI(namespace + node[2].getLabel());
			}
		...
		try {
			LdpUtils.exportIteration(writer, new URIImpl(baseUri.stringValue()), statements);
